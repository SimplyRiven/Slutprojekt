...
<body>
	...
	<div id="summaries">
		...
		if ($_GET != null) 
		{
			...
			$statement = $GLOBALS["pdo"]->prepare("SELECT * FROM summaries WHERE id=:summary_id");
			...
			if ($statement->execute())
			{
				while ($summarydisplay = $statement->fetch()) 
				{
					if ($statement->rowCount() > 0)
						echo "<h1>{$summarydisplay['title']}</h1><p>Tillagd: {$summarydisplay['date']}</p><p>{$summarydisplay['content']}</p>";
				}
			}
