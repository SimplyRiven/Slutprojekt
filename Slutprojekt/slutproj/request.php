...
<body>
		...
		if(!empty($_POST))
			{
				if(isset($_POST["title"]))
				{
					...
					$request = filter_input(INPUT_POST, 'request', FILTER_SANITIZE_SPECIAL_CHARS, FILTER_FLAG_STRIP_LOW);
					$title = filter_input(INPUT_POST, 'title');
					$course_id = filter_input(INPUT_POST, 'course_id');
					$statement = $pdo->prepare("INSERT INTO requests (user_id, course_id, request, title) VALUES (:user_id, :course_id, :request, :title)");
	...
	<div class="description">
		<h1> Just nu lägger du till en förfrågan </h1>
		<span> Var vänlig och välj under vilket namn och vilken kurs du vill att förfrågan skall tillhöra </span>
	</div>
	<form id="postrequest" action="request.php" method="POST">
			...
			<p>
				...
				<textarea name="request" rows="10" cols="70"></textarea>
